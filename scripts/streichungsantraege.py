...
from ekklesia_portal.datamodel import Department, Ballot, Proposition, Tag, PropositionType, Document, Changeset, \
...
def read_election_program(filepath):
...
@log_call
def parse_election_program(content, layer_depth):
...
@log_call
def insert_proposition(subject_area, proposition_type, voting_phase, proposition, identifier, data_tag_regex, document_id):
    ...
    section = proposition["number"]

    # Make title
    title = f'Streichung: {section} {proposition["name"]}'
    ...
    tags = ["wp", "bundestagswahl", "streichung"]
    ...
    proposition_obj.changesets.append(Changeset(document_id=document_id, section=section))
...
def insert_election_program(department_name, subject_area_name, voting_phase_name, propositions, number_start):
    ...
    for proposition in propositions:
        if insert_proposition(subject_area, proposition_type, voting_phase, proposition, id, data_tag_regex, args.document):
            id += 1
...
parser.add_argument("--document", help=f"Document ID", required=True)
parser.add_argument("--update-document", action="store_true", help=f"Update Document?", default=False)
...
if __name__ == "__main__":
    ...
    content = read_election_program(args.filepath)

    document: Document = session.query(Document).get(args.document)
    if document is None:
        raise ValueError(f"Document {args.document} not found!")

    if args.update_document:
        document.text = content

    propositions = parse_election_program(content, int(args.layer_depth))

    insert_election_program(args.department, args.subject_area, args.voting_phase, propositions, int(args.number_start))
